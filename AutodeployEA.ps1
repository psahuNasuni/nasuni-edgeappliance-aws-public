...
if ($GetSerialFromNMC -eq 'true') {
    ...
    write-output "Skipping Part1: NMC Login-getting Serial Numbers from the NMC is disabled"
}
...
if ($PageStateVar -like '*/wizard/network*' ) {
    ...
    write-output "FIler network configuration"
    # exit 1
}
...
if ($DomainJoin -eq 'true') {
    ...
    if ($JsonHealthStatus.healthy -eq 'True') {
        ...
        if ($JsonHealthStatus.healthy -eq 'True') {
            ...
            #Join Domain again if AD join health check fails
    ...
    write-output "Skipping domain join, domain join disabled in script"
}
